...
router.prefix('/s_flow');
router.all('/:token', async (ctx) => {
    let { uuid, url, kind, id, stime, etime } = tool_1.tool.getParamsAdmin(ctx);
    if (uuid == null && url == null && kind == null && id == null && stime == null && etime == null) {
        let back = await s_game_1.default.allOut(ctx, []);
        await ctx.render('a_flow', back);
        return;
    }
    let sql = {};
    if (uuid != "") {
    ...
    if (stime == "" && etime == "") {
        sql["time"] = { $gt: game_1.default.getNowTime() - 86400, $lt: game_1.default.getNowTime() };
    }
    if (stime != "" && etime == "") {
    ...
    let list = await mongodb_1.dbSev.getFlowDb().find("flow1", sql);
    let outf = [];
    for (const val of list) {
        val.time = game_1.default.getTimeS(val.time);
        outf.push(val);
    }
    let back = await s_game_1.default.allOut(ctx, outf, [uuid, url, kind, id, stime, etime]);
    await ctx.render('a_flow', back);
});
//# sourceMappingURL=s_flow.js.map