...
exports.ActDongTianPvdtModel = void 0;
 ...
 * pve 战斗
 */
class ActDongTianPvdtModel extends AModel_1.AModel {
    constructor() {
        super(...arguments);
        this.table = "act"; //数据库表名
        this.kid = "actDongTianPvdt"; //用于存储key 和  输出1级key
    ...
    */
    async fight_one() {
        ...
        info.nowId = actDongTian.pvdt; //当前在打的关卡ID
        let userModel = UserModel_1.UserModel.getInstance(this.ctx, this.id);
        let gStart = await userModel.getFightEps();
        //敌方阵容
        let cfgPvdt = gameCfg_1.default.dongtianPvdt.getItem(info.nowId.toString());
        if (cfgPvdt == null) {
            this.ctx.throw("已达关卡上限");
            return;
        }
        let cfgMonInfo = gameCfg_1.default.monDongtian.getItemCtx(this.ctx, cfgPvdt.mid.toString());
        info.start = {
            from: "dongtian",
        ...
        if (info.end.win == 1) {
            info.end.items = game_1.default.addArr(info.end.items, cfgPvdt.winItems);
            await actDongTianModel.pvdtPass(); //过关
        }
        else {
            // //活动 - 签到
...
exports.ActDongTianPvdtModel = ActDongTianPvdtModel;
//# sourceMappingURL=ActDongTianPvdtModel.js.map